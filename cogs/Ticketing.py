...
class TicketingSetup(commands.Cog):
    ...
    async def settings(self, ctx):
        embedSettings = discord.Embed(
            description="What do you want to do to this ticket?"
        )
    ...
    async def setdashboard(self, ctx, channel: discord.TextChannel = None):
        ...
        if channel is None:
            ...
            embedSuccess = discord.Embed(
                description=f"``{ctx.channel.mention}`` is now set as the ticket dashboard.",
                color=0xB50000,
            )
            ...
            if channel.id is olddashboard:
                embedError = discord.Embed(
                    description=f"Error! {channel.mention} is already set as the ticket dashboard",
                    color=0xB50000,
                )
                ...
                embedSuccess = discord.Embed(
                    description=f"``{channel.mention}`` is now set as the ticket dashboard.",
                    color=0xB50000,
                )
                await ctx.send(embed=embedSuccess)
