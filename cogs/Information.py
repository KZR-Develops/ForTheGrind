...
class Information(commands.Cog):
    ...
    async def server(self, ctx):
        guild = ctx.guild
        embedInfo = discord.Embed(
            description=guild.description, color=discord.Color.blue()
        )
        ...
        embedInfo.add_field(
            name="Channels",
            value=f"Text Channels: {len([channel for channel in guild.channels if isinstance(channel, discord.TextChannel)])}\nVoice Channels: {len([channel for channel in guild.channels if isinstance(channel, discord.VoiceChannel)])}",
            inline=False,
        )
        embedInfo.add_field(
            name="Boost Status",
        ...
        embedInfo.add_field(
            name="Creation Date",
            value=guild.created_at.__format__("%B %d, %Y @ %I:%M %p"),
            inline=False,
        )

        if guild.verification_level == discord.VerificationLevel.none:
            embedInfo.add_field(
                name="Verification Level", value="[None] This server is unrestricted."
            )
        elif guild.verification_level == discord.VerificationLevel.low:
            embedInfo.add_field(
                name="Verification Level",
                value="[Low] Must have a verified email on their account.",
            )
        elif guild.verification_level == discord.VerificationLevel.medium:
            embedInfo.add_field(
                name="Verification Level",
                value="[Medium] Must be registered for longer than 5 minutes.",
            )
        elif guild.verification_level == discord.VerificationLevel.high:
            embedInfo.add_field(
                name="Verification Level",
                value="[High] Must be a member for longer than 10 minutes.",
            )
        elif guild.verification_level == discord.VerificationLevel.highest:
            embedInfo.add_field(
                name="Verification Level",
                value="[Highest] Must have a verified phone on their account.",
            )
