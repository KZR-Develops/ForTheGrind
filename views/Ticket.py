...
async def ticketDashboard(type, claimStatus, interaction: discord.Interaction):
    ...
    dashboard = discord.utils.get(interaction.guild.channels, id=config["channels"]["ticket_dashboard"])
...
async def generate(self, interaction: discord.Interaction, type):
        ...
        if type == "question":
            ...
            await ticketDashboard(interaction=interaction, type="Question", claimStatus="Open")
...
class closedButtons(discord.ui.View):
    def __init__(self, interaction: discord.Interaction):
        ...
        @discord.ui.button(label="Open Transcript", style=discord.ButtonStyle.link, custom_id="cB:trans:link")
        async def transcript(self, interaction: discord.Interaction):
            ...
            if os.path.exists(f'./extras/transcripts/{ticketID}.md'):
                with open(f'./extras/transcripts/{interaction.channel.id}.md', 'rb') as f:
                    ...
                    embedSending = discord.Embed(description="File is being generated, it will be directly sent to you in a moment.", color=0xb50000)
                    await interaction.response.send_message(embed=embedSending, ephemeral=True)
                    await interaction.user.send(file=file)
            else:
                embedError = discord.Embed(description="Transcript was not found, it may have been deleted.", color=0xb50000)
...
class Feedback(discord.ui.View):
    ...
    @discord.ui.button(label="Write a feedback", style=discord.ButtonStyle.blurple, custom_id="feedback:blurple")
...
async def feedbackDashboard(guild: discord.Interaction, feedback, interaction: discord.Interaction, rating):
...
class FeedbackModal(discord.ui.Modal, title="Send us your feedback"):
    def __init__(self, title="Send us your feedback", guild: discord.Interaction=None):
    ...
    async def on_submit(self, interaction: Interaction):
        ...
        await interaction.response.send_message(embed=embedSuccess, view=feedbackRating(guild=self.guild, feedback=self.feedback.value, interaction=interaction))
...
class feedbackRating(discord.ui.View):
    ...
    options = [
        discord.SelectOption(label="Very Good", value=5, description="The issue was resolved exceptionally well, and the service provided exceeded expectations."),
        discord.SelectOption(label="Good", value=4, description="The issue was resolved satisfactorily, and the service provided was of a good standard."),
        discord.SelectOption(label="Ok", value=3, description="The resolution of the issue was average, meeting the basic requirements."),
        discord.SelectOption(label="Bad", value=2, description="The resolution of the issue was unsatisfactory, and improvements are needed in the service provided."),
        discord.SelectOption(label="Very Bad", value=1, description="The issue was not resolved effectively, and the service provided was extremely poor.")
    ]
    ...
    async def menu_callback(self, interaction: discord.Interaction, select: discord.ui.Select):
        ...
        if select.values[0] == "5":
            await feedbackDashboard(guild=self.guild, feedback=self.feedback, rating=5, interaction=interaction)
        if select.values[0] == "4":
            await feedbackDashboard(guild=self.guild, feedback=self.feedback, rating=4, interaction=interaction)
        if select.values[0] == "3":
            await feedbackDashboard(guild=self.guild, feedback=self.feedback, rating=3, interaction=interaction)
        if select.values[0] == "2":
            await feedbackDashboard(guild=self.guild, feedback=self.feedback, rating=2, interaction=interaction)
        if select.values[0] == "1":
            await feedbackDashboard(guild=self.guild, feedback=self.feedback, rating=1, interaction=interaction)
